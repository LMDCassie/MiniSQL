...
string delete_value[32];
int delete_num;
vector<string>select_value;
int select_num;

BufferManager mybuffer;

vector<int>select_offset;

char* convert_value(Tuple &tuple){
	string value="";
	char *p=new char[2+tuple.length()];
	memset(p,0,2+tuple.length());
	char *v=p;
	...
	for(int i=0;i<tuple.attr_count;i++){
		if(tuple.attrs[i].attr_type==INT){
            ivalue= toi(tuple.attr_values[i].c_str());
            memcpy(v,&ivalue,4);
            v+=4;
		}
...
bool Delete_tuple(vector<int> offsetlist,Table table,Condition_list list){
    ...
    if(Blocknum!=0||offsetlist.empty()!=1){
        while(true){
            ...
            if(offset>=Block_num(file_name))break;
            Target=mybuffer.GetBlock(file_name,offset,0);
                    ...
                    if(list.empty()!=1){
                        for(li=list.begin();li!=list.end();li++){
                            for(i=0;i<table.attr_count;i++){
                                if(table.attrs[i].attr_name==li->attr_name)break;
                            }
                            ip=0;
                            for(j=0;j<i;j++){
            ...
            if(offsetlist.empty()==1){
                offset++;
                if(offset==Blocknum)break;
            }
            if(offsetlist.empty()!=1){ 
                vi++; 
                if(vi == offsetlist.size())break;
            } 
        }		
    }
    delete target;
